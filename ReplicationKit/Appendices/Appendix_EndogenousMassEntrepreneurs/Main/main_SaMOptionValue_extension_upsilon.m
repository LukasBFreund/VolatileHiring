...
% Extended version: endogenous mass of firms given fixed entry cost
...
% Note that this recalibration is done imposing the standard model
...
FigName = 'fig_App_SaM_HetFirm_EndoMass_Isoelastic_iota100';
sPlotting.IrfPeriodsNum = 10;
sPlotting.numSubplotV = 4;
sPlotting.numSubplotH = 2;
sPlotting.variables = {'JaHat_aHatSS','JU','p','aStar','Upsilon','h','g','u'}; 
sPlotting.names = {'Value of entrepreneur at steady-state cutoff level','Value of unmatched entrepreneur',...
         'Entry probability','Cond. expected. productivity draw','Mass of potential entrepreneurs',...
         'Hiring rate','Entry probability: entrepreneurs','Unemployment rate'};
...
% mu = g(Upsilon) J^U
% Here impose: 
% g(Upsilon) = Upsilon^(-iota) 

% Fix iota
sPar.iota = 100; % cost mu will be pinned down below
...
switch optionRecalibration 
    case 'no'
        ...
        sPar.Pss = pTarget;
        sPar.Upsilon = vss/sPar.Pss + (1-sPar.delta)*nss; % to change as little as possible in code, keep in parameter vector
...
%---------------------------------------------------------------------------
    ...
    %% Endogenous mass of entrepreneurs: pin down cost function given
    % targeted ss-mass of entrepreneurs, value of being unmatched
    % and functional form of probability
    sPar.mu = sPar.Upsilon^(-sPar.iota)*JU;
    
    % Put everything together into a sub-element of the parameter structure 
    % (called by Dynare)
    sPar.ss.Upsilon = sPar.Upsilon; % will now be treated as endogenous
    sPar.ss.n = n;
    sPar.ss.m = sPar.delta*sPar.ss.n;
    sPar.ss.us = 1-(1-sPar.delta)*sPar.ss.n;
    sPar.ss.v = v;
...
gPos = strmatch('g',vNames,'exact'); % entry probability by entrepreneurs
...
sResults.aIRF(:,JUPos,:) = sResults.aIRF(:,JUPos,:)*vEMAS(JUPos);
