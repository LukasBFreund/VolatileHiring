...
%
% Model version: infinitely lived entrepreneurs, special case of gamma = 0
% (special case is easier & faster to solve)
%
% Calls: - dynare mod file + steady-state file
%        - fn_omega_chi & fn_omega_chi_nodeath
%        - fn_Cutoff_nodeath
%        - fn_Elasticity and fn_Elasticity_nodeath
%
...
% Choose whether to apply the recalibration method given this model's
% equations or, instead, using those of the model with entrepreneur death.
% The methodologically consistent approach is, of course, the former, 
% but we'll also look at the latter for illustrative purposes (i.e.,
% such that a comparison b/w models with and without death share the 
% same parameters; even if that implies that we miss targets in the
% model w/o death).
optionRecalibration_type = 'no_death'; % 'no_death' or 'death'

% Idiosyncratic dipersion
sPar.sigma_a = 0.025;  % max value higher (~0.025) than in baseline (~0.003)
...
FigName = 'fig_SaM_HetFirm_p05_NoDeath_sigmaa0025_Recalib';
...
switch optionRecalibration 
    ...
    case 'yes' 
        ...
        % probability -- no need to adjust here given gamma = 0
        ...
        % NB: the function called below is adjusted for the 'no death' formulation
        ...
        switch optionRecalibration_type
            case 'no_death'              
                [vSolOmegaChi0,~,~] = fsolve(@(x) fn_omega_chi_nodeath(x,sPar,target),vInitOmegaChi,options);
                [vSolOmegaChi,~,exitflag] = fsolve(@(x) fn_omega_chi_nodeath(x,sPar,target),vSolOmegaChi0,options);
            case 'death'
                [vSolOmegaChi0,~,~] = fsolve(@(x) fn_omega_chi(x,sPar,target),vInitOmegaChi,options);
                [vSolOmegaChi,~,exitflag] = fsolve(@(x) fn_omega_chi(x,sPar,target),vSolOmegaChi0,options);
        end
...
vSol = fsolve(@(x) fn_Cutoff_nodeath(x,sPar),vInit,options);
vSol = real(vSol);
...
checkCutoff = -sPar.kappa/h  + (LambdaaStar + sPar.beta*(1-sPar.delta)*p*sPar.kappa) ...
...
% Double-check elasticity of h w.r.t. z
disp('Difference between elasticity and target')
disp(abs(target-fn_Elasticity_nodeath([sPar.omega sPar.chi],sPar)))
