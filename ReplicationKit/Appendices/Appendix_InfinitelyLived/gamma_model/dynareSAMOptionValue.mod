...
// The following are new to this model variant 
    Gammma      ${\Gamma}$ (long_name='Measure of entrepreneurs making new draws')
    s           ${\Gamma}$ (long_name='Measure of unavailable entrepreneurs')
    JUtilde     ${\tilde{J}^U}$ (long_name='Value of newly separated entrepreneur')
...
set_param_value('gammma',sPar.gamma);
...
JaHatPTilde = (1-omega)*(xss*z-chi)+betta*((1-delta)*JaHatPTilde(+1)+delta*JUtilde(+1));


[name = 'Expected value of newly matched firm']
JStar = ((1-omega)*(xss*aStar))/(1-betta*(1-delta)) + JStarPTilde;
JStarPTilde = (1-omega)*(xss*z-chi)+betta*((1-delta)*JStarPTilde(+1)+delta*JUtilde(+1));
...
v = p*Gammma;
//v = (Upsilon-(1-delta)*n(-1));

[name = 'Measure of entrepreneurs with new draws']
Gammma = Upsilon-(1-delta)*n(-1) - delta*gammma*s(-1);

[name = 'Measure of unavailable entrepreneurs']
s = n + gammma*s(-1);

[name = 'Value of newly separated entrepreneur']
JUtilde = (1-gammma)*JU + gammma*betta*JUtilde(+1);
...
JaHatPTilde_aHatSS = (1-omega)*(xss*z-chi)+betta*((1-delta)*JaHatPTilde_aHatSS(+1)+delta*JUtilde(+1));
...
save('Output\IRFs\IRFs_HetFirm_GammaModel_Spc', 'vEMAS', 'vNames', 'vNames_Andreasen', 'mIRFProp_zUncertainty_EMAS', 'mIRFProp_Andreasen_zVol_EMAS','mIRFProp_Andreasen_z_EMAS','mIRFProp_Andreasen_Order1_z_EMAS','mIRFProp_Order1_z_EMAS','mIRFProp_Andreasen_Order3_z_EMAS'); %,'sIRFDynare')
