...
% Model version: *normal* distribution, additive productivity 
...
% Notes:
% - integrates sigma_a = 0 (separate Dynare file)
% - presumes that we have Andreasen et al.'s pruning code in the folder
%
...
sigma_a_low = 0.001;    % lowest value greater than 0 is ~0.001
sigma_a_high = 0.001;    
...
FigName = 'fig_SaMOptionValue_NormalDist_p05_sigmaa001_Recalib';
...
sPlotting.numSubplotH = 2;
 ...
 sPlotting.variables = {'JU_d','u','p','aStar'}; 
 sPlotting.names = {'Value of unmatched entrepreneur','Unemployment rate',...
         'Entry probability','Cond. expected. productivity draw'};
...
sPar.pii = pi; % avoids issue with some versions of dynare
...
if numel(vSigma_a)>1
    %xStep = 0 +  vSigma_a(2)-vSigma_a(1); % step-size for xlabel
...
switch optionRecalibration 
    ...
    case 'yes' 
        ...
        % uniform distribution formulas => switch to normal
        sPar.ss.aHat  = norminv(1- sPar.ss.p,sPar.aMean,sPar.sigma_a); 
        normpdfEval = ((1/sqrt(2*pi))*exp(-0.5*((sPar.ss.aHat-sPar.aMean)/sPar.sigma_a).^2));
        sPar.ss.aStar = sPar.aMean+sPar.sigma_a*normpdfEval/sPar.ss.p;
        
%        % Uniform
%        sPar.uni = sPar.sigma_a/(sqrt(1/3));
%        sPar.aH = sPar.aMean + sPar.uni;
%        sPar.aL = sPar.aMean - sPar.uni;  
%        aStar_Uniform = 0.5*(sPar.aH+sPar.ss.aHat)
        ...
        omega0 = (w0-chi0)/(sPar.xss*sPar.ss.z-chi0); 
           ...
           options = optimoptions('fsolve','Display','iter','Algorithm','trust-region',...
            ...
            test = fnElasticity([sPar.omega,sPar.chi],sPar);                    
end
...
%---------------------------------------------------------------------------
    ...
    sPar.ss.normpdfEval = normpdfEval; 
...
%---------------------------------------------------------------------------
    ...
    sResults.vpss(iS) = sPar.ss.p;
    sResults.sPar(iS) = sPar;
    aStore(:,:,:,iS) =  sResults.aIRF(:,:,:); % full IRFs
    if sPar.sigma_a >0
        sResults.vElasticity(iS) = fnElasticity([sPar.omega sPar.chi],sPar); % numerical, given normal
    elseif sPar.sigma_a == 0
        sResults.vElasticity = -sPar.TZElasticity*sPar.alpha;
    end
end

ssResults.names = vNames;
