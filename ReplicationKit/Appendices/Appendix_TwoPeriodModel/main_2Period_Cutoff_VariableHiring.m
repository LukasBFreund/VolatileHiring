...
%
% Plot cutoff determination with variable hiring rate
%
...
EProfits2 = 0.5*(p2G*subs(profits2G,a,aStar2G))...
            + 0.5*(p2B*subs(profits2B,a,aStar2B)); % 0.5*EProfits2_G_Uncond+0.5*EProfits2_B_Uncond
...
set(gca,'XTick',[xLow:(xHigh-xLow)/3:xHigh],'FontSize',sSettings.font.size.axisticks,'fontname',sSettings.font.name);
...
text(vaHat1_Sol(1)-0.003,+0.004,txt,'Interpreter','latex','FontSize',sSettings.font.size.axisticks,'fontname',sSettings.font.name);
txt = '$\swarrow \hat{a}_1 (\Delta > 0)$';
text(vaHat1_Sol(end)+0.0005,+0.005,txt,'Interpreter','latex','FontSize',sSettings.font.size.axisticks,'fontname',sSettings.font.name);
...
vline(aBar,':k');
txt=('$\leftarrow \bar{a}$');
text(0.1,0.05,txt,'Interpreter','latex','color',sSettings.colors.list{1},'FontSize',6,'fontname',sSettings.font.name);
  ax = gca;
        ax.XAxis.Exponent = 0; 
     
if optionPrint == 1
print(fig1,'Output\fig_App_2P_CutoffDetermination_hVariable_alpha05_hBar1_aBar01_Delta015','-dpdf','-painters')
end
