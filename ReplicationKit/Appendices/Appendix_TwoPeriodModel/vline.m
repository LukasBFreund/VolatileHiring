...
% function h=vline(x, linetype, label,font)
% 
...
if length(x)>1  % vector input
    ...
    if length(label)
        ...
        xunit=(x-xx(1))/xrange;
        if xunit<0.8
            text(x+0.01*xrange,y(1)+0.1*(y(2)-y(1)),label,'color',get(h,'color'),'fontname','times')
        else
            text(x-.05*xrange,y(1)+0.1*(y(2)-y(1)),label,'color',get(h,'color'),'fontname','times')
        end
    end     
    if g==0
    hold off
